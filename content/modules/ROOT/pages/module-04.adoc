...
+
. Before we make *node-affinity-vm* move because of our new Node Affinity rule. Let's check its current location by looking at OpenShift Console and navigating to *Virtualization →  VirtualMachines → affinity* or by checking on the OpenShift Command line.
