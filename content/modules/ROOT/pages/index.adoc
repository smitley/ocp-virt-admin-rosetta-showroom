...
The cluster Web Console is accessible at: {openshift_cluster_console_url}[{openshift_cluster_console_url},window=_blank]


The username is `{openshift_cluster_admin_username}` and the password is `{openshift_cluster_admin_password}`.


The cluster API is accessible at `{openshift_api_server_url}`.
